<?php
// mn-filter section of the plugin// easier to work with includes
// mn-filter start tag
echo '<div class="mn-filter">';
    // get mn-category ID from slug
    // check if taxonomy-term is set
    if ( $mn_taxonomy_term !== 'unset' ) {
        $mn_term = get_term_by('slug', $mn_taxonomy_term, $mn_taxonomy);        $mn_term_id = $mn_term->term_id;        $mn_term_children = get_term_children($mn_term_id, $mn_taxonomy);
    } else {
        $mn_terms = get_terms();        $mn_term_children = [];
    ...
    if ( $mn_term_children ) {
        ...
        echo '<ul>';
            ...
            foreach ( $mn_term_children as $mn_term_child ) {
                // get term slug for #link                // get term name
                $child_term = get_term_by('id', $mn_term_child, $mn_taxonomy);                $child_term_slug = $child_term->slug;                $child_term_name = $child_term->name;
                // item start tag
                echo '<li>';
